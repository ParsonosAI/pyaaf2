...
MediaContainerGUID = {
    "Generic": (
        AUID("b22697a2-3442-44e8-bb8f-7a1cd290ebf1"),
        (
            ".3g2",
            ".3gp",
            ".aac",
    ...
    "AVCHD": (
        AUID("f37d624b307d4ef59bebc539046cad54"),
        (
            ".mts",
            ".m2ts",
        ),
    ),
    "ImageSequencer": (AUID("4964178d-b3d5-485f-8e98-beb89d92a5f4"), (".dpx",)),
    "CanonRaw": (AUID("0f299461-ee19-459f-8ae6-93e65c76a892"), (".rmf",)),
    "WaveAiff": (
        AUID("3711d3cc-62d0-49d7-b0ae-c118101d1a16"),
        (
            ".wav",
            ".wave",
            ".bwf",
    ...
    "MXF": (AUID("60eb8921-2a02-4406-891c-d9b6a6ae0645"), (".mxf",)),
    "QuickTime": (
        AUID("781f84b7-b989-4534-8a07-c595cb9a6fb8"),
        (
            ".mov",
            ".mp4",
            ".m4v",
    ...
    with open(path, "rb") as file:
        ...
        while True:
            ...
            if chunkid[0:3] != b"fmt":
                ...
                return bytearray(
                    b"RIFF" + data_size + b"WAVE" + chunkid + sizebuf + file.read(size)
                )
...
def create_network_locator(f, absolute_path):
    ...
    if sys.version_info[0] < 3:
        ...
        n["URLString"].value = absolute_path.as_uri()
...
class StreamInfo:
    ...
    def create_pcm_descriptor(self, f):
        ...
        bit_depth, block_align = audio.audio_format_sizes.get(
            self.metadata["sample_fmt"], (0, 0)
        )
...
def _create_ama_mobs(f, name, format_info, source_descriptor, metadata):
    ...
    def append_source_to_mob_as_new_slots(from_mob, to_mob):
        ...
        for from_slot in from_mob.slots:
            ...
            if slot_kind in ("Picture", "Sound"):
                from_clip = from_mob.create_source_clip(
                    slot_id=from_slot.slot_id, media_kind=slot_kind
                )
                to_slot = to_mob.create_empty_sequence_slot(
                    edit_rate=from_slot.edit_rate, media_kind=from_clip.media_kind
                )
    ...
    def source_mob_from_tape_mob(name, tape_mob):
        ...
        try:
            source_mob.mob_id = MobID(
                int=int(metadata["format"]["tags"]["umid"], base=16)
            )
        except KeyError:
            pass
